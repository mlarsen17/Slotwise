...
def compute_cohort_baselines(
    ...
    *,
    ...
    """Compute snapshot-safe cohort baselines from data known at effective_ts.

    Historical cohort metrics are computed from information available at snapshot
    time; no post-snapshot events may affect cohort baselines.

    Rule: slots that are visible by effective_ts are included in cohort training
    regardless of whether slot_start_at is before or after effective_ts.
    """
    slots = conn.execute(
        """
    ...
    if slots.empty:
        ...
        if slots.empty:
            ...
            completed_slots = set(completed["slot_id"].tolist()) if not completed.empty else set()
            ...
            active_end = slots[["unavailable_at"]].copy()
            active_end["unavailable_at"] = active_end["unavailable_at"].fillna(
                slots["slot_start_at"]
            )
            active_hours = (
                pd.concat(
                    [active_end["unavailable_at"], pd.Series(snapshot_ts, index=slots.index)],
                    axis=1,
                ).min(axis=1)
                - slots["visible_at"]
            ).dt.total_seconds() / 3600
    ...
    if dupes:
        raise ValueError("Duplicate cohort baseline rows detected")
    return baselines
