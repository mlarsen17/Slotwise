...
def load_core_tables(conn: duckdb.DuckDBPyConnection, normalized: dict[str, pd.DataFrame], scenario_id: str) -> None:
    ...
    for table in ["businesses", "providers", "slots", "booking_events"]:
        conn.execute(f"DELETE FROM {table} WHERE scenario_id = ?", [scenario_id])

    conn.execute("INSERT INTO businesses SELECT business_id, source_business_id, name, scenario_id FROM tmp_businesses")
    conn.execute(
        "INSERT INTO providers SELECT provider_id, business_id, source_provider_id, source_business_id, name, scenario_id FROM tmp_providers"
    )
    conn.execute(
        """
        INSERT INTO slots (
          slot_id, provider_id, business_id, source_slot_id, source_provider_id, source_business_id,
          slot_start_at, slot_end_at, visible_at, unavailable_at, current_status, created_at,
          source_system, source_run_id, scenario_id
        )
        SELECT
          slot_id, provider_id, business_id, source_slot_id, source_provider_id, source_business_id,
          slot_start_at::TIMESTAMP, slot_end_at::TIMESTAMP, visible_at::TIMESTAMP, NULL, NULL,
          created_at::TIMESTAMP, source_system, source_run_id, scenario_id
        FROM tmp_slots
        """
    )
    conn.execute(
        ...
        SELECT
          event_id, slot_id, source_event_id, source_slot_id, event_type,
          event_at::TIMESTAMP, source_system, source_run_id, scenario_id
        FROM tmp_events
        """
    )

    slot_count = conn.execute("SELECT COUNT(*) FROM slots WHERE scenario_id = ?", [scenario_id]).fetchone()[0]
